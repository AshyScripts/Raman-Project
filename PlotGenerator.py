...
def plot_dendro(X, labels, metric="euclidean"):
    ...
    Z = dendrogram(Y, ax=ax_dendro, labels=labels, leaf_font_size=30, leaf_rotation=30)
    ...
    # plt.rc('font', size=20)
    plt.savefig(
        "C:/Users/ashka/Desktop/raman/figures/dendro.png", dpi=600, bbox_inches="tight"
    )
    plt.savefig("C:/Users/ashka/Desktop/raman/figures/dendro.pdf", bbox_inches="tight")
...
def plot_outcome_vs_feature_ten_vs_tenf(feature, outcome, df, plot_type="scatter"):
    ...
    if plot_type == "scatter":
        ...
        tenf_plot = ax.scatter(tenf[feature], tenf[outcome], c="r", label="TenF")
    elif plot_type == "line":
        ten_plot = ax.scatter(ten[feature], ten[outcome], c="b", label="Ten")
        tenf_plot = ax.scatter(tenf[feature], tenf[outcome], c="r", label="TenF")
...
def plot_all_learning_curve(
    estimator, data_matrix, features=None, outcomes=None, sample_rng=None, cv=3
):
    ...
    for outcome in outcomes:
        X_train, y_train = data_matrix.gen_train(features, outcome, sample_rng)
...
def plot_features_scores(
    outcome, features_scores, message1=None, message2=None, shadow=None
):
    ...
    if message1 is not None:
        ...
        plt.annotate(message1, xy=(xcoord, ycoord), xytext=(xcoord_text, ycoord_text))
    if message2 is not None:
        xcoord = 2
        ycoord = scores[2]
        xcoord_text = xcoord + 0.05
        plt.annotate(message2, xy=(xcoord, ycoord), xytext=(xcoord_text, ycoord_text))
    # plt.ylim(np.min(scores) - 0.5, np.max(scores) + 0.5)

    plt.ylim(-1, 1)
    plt.xticks(x, labels, rotation=30)
    plt.errorbar(
        x, scores, fmt="-o", yerr=stds, capsize=20, markersize=20, color="yellowgreen"
    )
    if shadow is not None:
        plt.fill_between(
            x, scores - stds, scores + stds, alpha=0.1, color="yellowgreen"
        )
...
def plot_validation_curve(
    ...
    test_scores_std = np.std(test_scores, axis=1)
    plt.figure(figsize=(15, 10))
    plt.title("Validation Curve with " + estimator_name + " for " + outcome_name)
    ...
    if np.array_equal(
        np.linspace(param_range[0], param_range[-1], len(param_range)), param_range
    ):
        plt.plot(
            param_range,
            train_scores_mean,
