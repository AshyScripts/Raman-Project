...
def select_all(estimator, data_matrix, n_features):
    ...
    for outcome in data_matrix.outcomes:
        print(outcome)
        X_train, y_train = data_matrix.gen_train(
            data_matrix.features, outcome, data_matrix.sample_rng
        )
        selector = selector.fit(X_train, y_train)
        print([features[i] for i in np.arange(len(features)) if selector.support_[i]])
    ...
    while len(features_not_included) > 0:
        ...
        for feature in features_not_included:
            ...
            if param_name is not None:
                train_scores, test_scores = ModelSelection.tune_param(
                    None,
                    estimator,
                    param_name,
                    param_range,
                ...
                X_train_orig, y_train_orig = data_matrix.gen_train(features, outcome)
                ...
                for rep in np.arange(repetition):
                    ...
                    test_scores = cross_val_score(estimator, X_train, y_train, cv=cv)
