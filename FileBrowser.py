...
class FileBrowser(object):
    def __init__(self, root_dir=None, param_rule_file=None):
        if root_dir is None:
            self.root_dir = os.path.expanduser("C:/Users/ashka/Desktop/raman/")
        else:
            self.root_dir = root_dir
        if param_rule_file is None:
            param_rule_file = os.path.expanduser(
                "C:/Users/ashka/Desktop/raman/parameter_rules.csv"
            )
    ...
    def show_sample_with_experiments(self, sample_rng=np.arange(1, 10000)):
        all_experiment_types = map(
            str.strip, self.param_rule.ix["experimentType", "options"].split(",")
        )
        ...
        for sample_id in sample_rng:
            ...
            entry = [
                "Yes" if exp in experiment_types else "No"
                for exp in all_experiment_types
            ]
