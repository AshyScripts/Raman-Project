...
default_param_rule = pd.read_csv(
    os.path.expanduser("C:/Users/ashka/Desktop/raman/parameter_rules.csv"), index_col=1
)
experiment_types = default_param_rule.loc["experimentType", "options"].split(",")
...
experiment_types = experiment_types + ["genRF", "genR", "RF", "R"]

default_features = dict()
default_outcomes = dict()
default_input_dir = os.path.expanduser("C:/Users/ashka/Desktop/raman")
default_features["TenS"] = [
    u"density",
...
default_features["F"] = [u"voltage", u"freq", u"totalNumOfCycles"]
default_outcomes["F"] = [u"frequency", u"voltage", u"timeVolOnPerPul", u"dutyCycle"]
...
default_features["RF"] = [u"voltage", u"freq", u"totalNumOfCycles"]
...
class DataMatrix(object):
    def __init__(
        ...
        if data_frame is None:
            ...
            if experiment_type is None or sample_rng is None:
                raise Exception(
                    "Both experiment_type and sample_rng "
            ...
            if self.experiment_type not in experiment_types:
                raise Exception(
                    "Experiment type " + self.experiment_type + " is not supported"
                )
            ...
            if self.experiment_type in ["genRF", "genR"]:
                ...
                self.matlab_engine.addpath("C:/Users/ashka/Desktop/raman/RamanData")

            self.features = features if features else default_features[experiment_type]

            self.outcomes = outcomes if outcomes else default_outcomes[experiment_type]

            self.input_dir = input_dir if input_dir else default_input_dir

            self.param_rule = (
                pd.read_csv(param_rule_file, index_col=1)
                if param_rule_file
                else default_param_rule
            )
            ...
            if features is None or outcomes is None:
                raise Exception(
                    "Features, outcomes and sample range "
            ...
            self.sample_rng = (
                sample_rng if sample_rng is not None else data_frame["sampleID"].values
            )

        self.data_matrix_cleaned = self.data_matrix.dropna(axis=1, how="all")
        for col in self.features:
            if np.any(self.data_matrix_cleaned[col].isnull()):
                print("Feature " + col + " contains NaN, filled in with average value")
    ...
    def gen_train(self, features, label, sample_rng=None):
        if sample_rng is None:
            return self.matrix_generator.gen_train(features, label, self.sample_rng)
        else:
    ...
    def scaled_mat(self, features=None, outcomes=None):
        ...
        self.scaled_data_matrix = util.gen_normalized_dataset(
            self.data_matrix, features, outcomes
        )
    ...
    def gen_data_matrix(self):
        ...
        for sample_id in self.sample_rng:
            ...
            param_file = util.find_file_with_regex(
                self.input_dir, param_file_pattern, sample_id
            )
            if not param_file:
                print("param_file for sampleID: " + str(sample_id) + " does not exist")
                continue
            ...
            if self.experiment_type == "genR":
                ...
                data_file_pattern = (
                    self.experiment_type + "_" + str(sample_id) + "(_\d)*" + ".csv"
                )

            data_file = util.find_file_with_regex(
                self.input_dir, data_file_pattern, sample_id
            )
            if data_file is None:
                raise Exception("No files found with pattern: " + data_file_pattern)
        ...
        if len(df_list) != 0:
            dm = pd.concat(df_list, ignore_index=True).apply(
    ...
    def gen_raman_data_from_file(self, param_file, data_files):
        ...
        outcomes_path = os.path.join(base_dir, "Outcomes_" + str(sample_idx) + ".csv")
        if "gen" in self.experiment_type:
            ...
            raman_param = param[
                param["experimentType"].astype(str).isin(r_list)
            ...
            for file_path in data_files:
                ...
                for j, col_name in enumerate(self.outcomes):
                    raman_param.loc[
                        raman_param["experimentType"] == r_list[i], col_name
                    ] = raman_outcomes[j]
    ...
    def gen_fusion_data_from_file(self, param_file, data_file=None):
        ...
        if data_file:
            ...
            time_voltage_on_per_pulse = np.mean(
                times[even].as_matrix() - times[odd].as_matrix()
            )
            total_time_voltage_on = np.sum(
                times[even].as_matrix() - times[odd].as_matrix()
            )
    ...
    def pre_process_ten(self, data_file, ten_param_series):
        if self.experiment_type == "TenF" or self.experiment_type == "Ten":
            ...
            starting_point = np.nonzero(force > 0.05)[0][0]
            ...
            displacement = ten_data.displacement.as_matrix()[
    ...
    def gen_tensile_data_from_file(self, param_file, data_file=None):
        ...
        if data_file:
            if self.experiment_type == "Ten" or self.experiment_type == "TenF":
                [linear_density, area, stress, strain] = self.pre_process_ten(
                    data_file, ten_param_series
                )
            elif self.experiment_type == "TenS":
                [stress, strain] = self.pre_process_ten(data_file, ten_param_series)
            ...
            for data_idx in np.arange(interval_start, data_length - strain_interval):
            ...
            line_for_linear_strain = (
                linear_youngs_modulus * (strain - strain[stiffness_idx] - 0.002)
                + stress[stiffness_idx]
            )

            # Find the intersection point of the offset line to the original stress/strain line
            intersect_point_idx = np.nonzero((line_for_linear_strain - stress) >= 0)[0][
                0
            ]
...
dm = DataMatrix(experiment_type="genRF", sample_rng=list(range(1, 6)))
dm1 = dm.gen_raman_data_from_file(
    param_file="C:/Users/ashka/Desktop/raman/parameter_rules.csv",
    data_files=[
        "RF_305_1.csv",
        "RF_305_2.csv",
        "RF_305_3.csv",
