...
from PlotGenerator import plot_learning_curve
...
def gen_x(rng=np.arange(17, 10000)):
    paramFile = pd.read_csv("C:/Users/ashka/Desktop/raman/parameter_rules.csv")
    sampleList = []
    fList = []
    vList = []
    TList = []
    ...
    dList = []
    gasAtmList = []
    for root, dirs, filenames in os.walk("C:/Users/ashka/Desktop/raman/RamanData"):
        if len(filenames) > 1:
...
def gen_y(rng=np.arange(17, 10000)):
    return (
        ((gen_r_after(rng) - gen_r_before(rng)) / gen_r_before(rng)).as_matrix().ravel()
    )
        ...
        for filename in filenames:
            if "IVF" in filename:
                ...
                if sampleNum in rng:
                    ...
                    rList.append(r)
    df = pd.DataFrame({"sampleNum": sampleList, "resistance": rList})
...
def spline(X, y, features, outcome):
    ...
    for line in estimator.summary_feature_importances().split("\n"):
        ...
        if len(line_cleaned) == 2:
            print(line_cleaned[0], line_cleaned[1], feature_s[line_cleaned[0]])
